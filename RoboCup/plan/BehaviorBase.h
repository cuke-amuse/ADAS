...
public:
  ...
  bool operator>(const ActiveBehavior &other) const {
    return this->mEvaluation > other.mEvaluation;
  }
  bool operator==(const ActiveBehavior &other) const {
    return this->mEvaluation == other.mEvaluation;
  }
...
public:
  explicit BehaviorPlannerBase(Agent &agent) : BehaviorDataType(agent) {}
...
public:
  ...
  virtual void
  SubmitVisualRequest(const ActiveBehavior &act_bhv, double plus = 0.0) { //但可以有多个行为提交视觉请求
    (void)act_bhv;
    (void)plus;
  }
