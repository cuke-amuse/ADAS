                             ...
                             const geometry_msgs::Pose& goal_pose)
{
  ...
  for (const auto& ll : continuous_lane.lanelets()) {
    ...
    const auto centerline = generateCenterline(ll, laneWidth);
...
lanelet::LineString3d RoutePlanner::generateCenterline(const lanelet::ConstLanelet& lanelet_obj, double& laneWidth)
{
  ...
  laneWidth = left_length + right_length;
